...
$SiteName = "Website1"                                        #<---------Name of the website in IIS
$WebsitePath = "http://localhost:8090/"                       #<---------Port number to check if website is online
$AppPoolName = "DefaultSiteAppPool"                           #<---------Name of the Application Pool
$build_folder="C:\inetpub\wwwroot\"                           #<---------Not needed?
$public=""                                                    #<---------Relative path pf public website; relative to build folder as asked below (old_build_folder/new_build_folder)
$globalerror = 0                                              #<---------This variable tracks if error(s) occured during the script
$PSEmailServer = "smtp.blahblah.com"                          #<---------This variable sets SMTP server. It's a system variable.
...
$deploymentresult += Write-Output "`nStarting deployment on $env:computername"
...
if ($globalerror -eq 0)
    {
    $deploymentresult += Write-Output  "`nOperation completed without any errors!"
    }
...
Send-MailMessage -To "me@me.com" -From "me@me.com" -Subject "Deployment result" -Body $deploymentresult
